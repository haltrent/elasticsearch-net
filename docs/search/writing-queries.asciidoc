...
[NOTE]
--
Not all documents are returned in the one response; by default, only the first ten documents
...
The previous examples used the Fluent API to express the query. NEST also exposes an
