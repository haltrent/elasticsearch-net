...
namespace Tests.ClientConcepts.HighLevel
{
     ...
     * == Getting started
	 *
	 * NEST is the high level Elasticsearch .NET client that still maps very closely to the original Elasticsearch API.
	...
	public class GettingStarted
	{
		...
		public async Task Indexing()
		{
			...
			{
				Id = new Guid("00000000-0001-5eed-1ced-c0ffee000000"),
				FirstName = "Martijn",
				LastName = "Laarman"
			};
		 ...
		 *
		 * This will index the document to the endpoint `/people/person/00000000-0001-5eed-1ced-c0ffee000000`. NEST is smart enough to infer the index
		 * and type from the Person CLR type, and was able to infer the id for the document by looking for an `Id` property on the POCO. Take a look
		 * at <<ids-inference,Ids inference>> to see other ways in which NEST can be configured to infer an id for a document.
		 *
		 * IMPORTANT: By default, NEST camel cases the property names on the POCO when serializing the POCO into a JSON document to send to Elasticsearch.
